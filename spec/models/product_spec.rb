...
describe Product do
  ...
  describe '.filtere by title' do
    ...
    context 'when a "TV" title pattern is sent' do
      it 'should returns the 2 products matching' do
  ...
  describe '.above_or_equal_to_price' do
    ...
    it 'should returns the products which are above or equal to the price' do
  ...
  describe '.recent' do
    ...
    it 'should returns the most updated records' do
  ...
  describe '.search' do
    ...
    context 'when title "videogame" and "100" a min price are set' do
      it 'should returns the product1' do
    ...
    context 'when an empty hash is sent' do
      it 'should returns all the products' do
        expect(Product.search({})).to match_array([@product1, @product2, @product3, @product4])
      end
    end

    context 'when product_ids is present' do
      it 'should returns the product from the ids' do
        search_hash = { products_ids: [@product1.id, @product2.id]}
        founded_products = Product.search(search_hash)
        expect(founded_products).to match_array([@product1, @product2])
      end
    end
